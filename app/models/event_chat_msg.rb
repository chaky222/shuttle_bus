...
class EventChatMsg < ApplicationRecord
  belongs_to :event
  belongs_to :user
  # has_many :event_chat_msg_unreads, dependent: :destroy

  after_create :add_unreads_to_all_event_users


  def add_unreads_to_all_event_users
    event.users_with_ticket.each do |tuser|
      tuser.event_chat_msg_unreads.create!(event_chat_msg_id: id)
    end
  end

  def msg_text_html; msg_html.to_s; end

  def self.pack_by_src_ca_name_event_chat_msgs(msgs_in, current_user_id)
