module PubPartiesSearchHelper
  def self.get_events_collection(params)
  ...
  def self.get_search_where_sql(search_text, show_hidden = false)
    ...
    default_fields = ['events.search_text']
    ...
    # end
    # where_sql
    get_search_sql(search_text, default_fields, { init_sql: show_hidden ? '(1)' : '(events.visibility_status > 0)' })
  end
  ...
  def self.get_search_sql(search_text, fields, params = {})
    ...
    search_text.split(/\s/i).each_with_index do |word, index|
      word = word.strip
      search_ands << { word: word, fields: fields } if word.length.positive?
    end
